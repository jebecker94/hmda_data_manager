...
dataset = HMDALoader.load_hmda_file(data_folder='data/clean', file_type='lar', min_year=2024, max_year=2024, engine='polars', filters=filters)

# Polars Filters
dataset = dataset.filter([pl.col('action_taken')==1])

# See how many total originations there are
print('Length:', dataset.select(pl.len()).collect())
...
df_weird = dataset.with_columns(
    ...
    pl.col('CountExemptions').mean().over(['lei','activity_year']).alias('AverageExemptions')
).filter(
    ...
    ['HMDAIndex','lei','activity_year'] + exempt_columns + ['CountExemptions', 'AverageExemptions']
)

# See How Many "Weird" Reporters there are
print('Length:', df_weird.select(pl.len()).collect())
